...
$FC_EXEC systemctl stop controller.service.service0.0
...
# $FC_SYSTEMCTL_STATUS controller.service.service0.0
# | grep criu-dump.log
# error_log=$($FC_SYSTEMCTL_STATUS controller.service.service0.0 | grep criu-dump.log | cut -d" " -f26)
# cut -d/ -f2- | cut -d ' ' -f1)
# cat "${error_log::-1}"
